...
data_dir <- "C:/Users/bai.li/Documents/Github/RMAS/vignettes/003_ASAPCaseStudy"
...
Compare temporal trends of biomass(B), abundance(A), spawning stock biomass (SSB), recruit (R), fishinig mortality (F), Landings (L), and Survey index (SI) from `ASAP` (dots) and `MAS` (lines).
```{r}
...
- Include parameter names for age-based selectivity in RMAS ouputs?
...
- Selectivity of fleet is age-based selectivity. In ASAP, the selectivities from age 7-10 are fixed at 1. In MAS, the selectivities from age 1-10 are estimated.

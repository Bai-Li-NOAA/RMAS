...
data_dir <- "C:/Users/bai.li/Documents/Github/RMAS/vignettes/004_SSCaseStudy"
ss_dat <- SS_readdat_3.30(file = file.path(data_dir, "data.ss"),
                          verbose = FALSE)

ss_ctl <- SS_readctl_3.30(file = file.path(data_dir, "control.ss"),
                          verbose = FALSE,
                          nseas = ss_dat$nseas,
                          N_areas = ss_dat$Nareas,
                          Nages = ss_dat$Nages,
                          Ngenders = ss_dat$Nsexes,
                          Nfleet = nrow(ss_dat$fleetinfo[ss_dat$fleetinfo$type==1,]),
                          Nsurveys = nrow(ss_dat$fleetinfo[ss_dat$fleetinfo$type==3,]),
                          Do_AgeKey = FALSE,
                          N_tag_groups = NA,
                          N_CPUE_obs = c(table(ss_dat$catch$V3),
                                         table(ss_dat$CPUE$index)),
                          use_datlist = FALSE,
                          datlist = NULL)
...
ss_wtatage <- SS_readwtatage(file.path(data_dir, "wtatage.ss"))
ss_starter <- SS_readstarter(file.path(data_dir, "starter.ss"), verbose = TRUE)
ss_forecast <- SS_readforecast(file.path(data_dir, "forecast.ss"), verbose = FALSE)
...
growth$SetMaleSurveyWeight(survey_waa[[1]][[2]]) # How to set emprical weight-at-age for two surveys? One survey unit is biomass and the other survey unit is number. The data are stored in survey_empirical_weight list.
...
for (j in 1:ngenders){
  ...
  initial_deviations[[j]]$values <- rep(0, times=nages)

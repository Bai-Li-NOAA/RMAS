---
title: "Example: Running MAS using operating model from the model comparison project"
...
If you receive errors related to C++ when loading RMAS, please see the installation instructions from the [main page](https://github.com/nmfs-fish-tools/RMAS#installation-instructions).

## 2. Load OM outputs
```{r}

data_path <- system.file("extdata", package="RMAS")
...
Still need to add biological reference points and selectivity-at-age to `MAS` outputs.
```{r}
...
mas$selexparm_fleet <- list(a50=parameter_table$Value[parameter_table$Parameter=="logistic_selectivity_a50_1"],
                       slope=parameter_table$Value[parameter_table$Parameter=="logistic_selectivity_slope_1"])
mas$selexparm_survey <- list(a50=parameter_table$Value[parameter_table$Parameter=="logistic_selectivity_a50_2"],
                       slope=parameter_table$Value[parameter_table$Parameter=="logistic_selectivity_slope_2"])

```

### Generate comparison figures
Compare temporal trends of biomass(B), abundance(A), spawning stock biomass (SSB), recruit (R), fishinig mortality (F), Landings (L), and Survey index (SI) from the OM (dots) and `MAS` (lines).
```{r}
...
Compare estimated R0, q, and selectivity parameters. The etimated selectivity slope values from the `MAS` were converted using 1/slope to match the slope from the OM. 
```{r}
var <- c("R0", "q", "selexparm_fleet", "selexparm_survey")
summary_table <- matrix(c(om$r0, mas$r0, 
